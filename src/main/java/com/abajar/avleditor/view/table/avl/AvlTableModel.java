...
package com.abajar.avleditor.view.table.avl;
 ...
 * @author hfreire
 */
public class AvlTableModel extends DefaultTableModel implements TableModelListener {

    public AvlTableModel getInitializedTable(){
    ...
    public AvlTableModel(Object model) {
        this.model = model;
    }

    protected AvlTableModel(){
        this.model = null;
    }
    ...
    protected void updateObject(TableModel tableModel) {
        ...
        for(Field field : this.model.getClass().getDeclaredFields()){
            if (field.isAnnotationPresent(AvlEditorField.class)){
                try {
                    field.setAccessible(true);
                    field.set(this.model, tableModel.getValueAt(0, fields));
                    fields++;
                } catch (IllegalArgumentException ex) {
                    Logger.getLogger(AvlTableModel.class.getName()).log(Level.SEVERE, null, ex);
    ...
    protected Object[][] getData() {
        ...
        for(Method method : this.model.getClass().getMethods()){
            if(method.isAnnotationPresent(AvlEditorReadOnly.class)){
                try {
    ...
    protected Object[] getColumns() {
        ...
        for(Field field : this.model.getClass().getDeclaredFields()){
            if (field.isAnnotationPresent(AvlEditorField.class)){
                AvlEditorField crrcsimFieldAnnotation = field.getAnnotation(AvlEditorField.class);
        ...
        for(Method method : this.model.getClass().getMethods()){
            if(method.isAnnotationPresent(AvlEditorReadOnly.class)){
                AvlEditorReadOnly crrcsimReadOnlyAnnotation = method.getAnnotation(AvlEditorReadOnly.class);
